...
import { useCollectionType } from "@/lib/strapi";
import Link from "next/link";
import Image from "next/image";
import RichContentRenderer from "@/app/components/RichTextRenderer";
...
export default function BlogPostClient({ slug }: { slug: string }) {
  ...
  // Get the post - first try from Strapi, then fallback to dummy posts
  const strapiPost =
  ...
  const postData = strapiPost || dummyPost || null;
        ...
        <article>
          ...
          {post.image && (
            <motion.div
              ...
              className="relative h-96 w-full mb-8 rounded-lg overflow-hidden bg-gray-100"
            >
              <Image
                src={post.image.url}
          ...
          >
            {post.article ? (
              Array.isArray(post.article) ? (
                <RichContentRenderer content={post.article} />
              ) : typeof post.article === "string" ? (
                <div
